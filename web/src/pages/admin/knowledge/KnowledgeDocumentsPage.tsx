...
export function KnowledgeDocumentsPage() {
  ...
  const [pipelineMap] = useState<Map<string, string>>(new Map());
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button size="icon" variant="ghost" className="h-8 w-8" title="更多" aria-label="更多操作">
...
function UploadDialog({ open, onOpenChange, onSubmit }: UploadDialogProps) {
  ...
  const [pipelines] = useState<IngestionPipeline[]>([]);
  const loadingPipelines = false;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>支持 TXT、Markdown、PDF、DOCX，并配置分块策略</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          {/*
