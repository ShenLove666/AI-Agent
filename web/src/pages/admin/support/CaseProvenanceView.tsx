...
export function CaseProvenanceView({ value }: { value: CaseProvenance | null }) {
  ...
  return <section aria-label="案例数据溯源" className="rounded-lg border border-slate-200 bg-slate-50 p-4"><div className="flex flex-wrap items-center gap-2"><strong className="text-sm">{value.dataSource.title}</strong>{value.isDemo && <Badge className="bg-amber-100 text-amber-900">DEMO 场景</Badge>}<Badge variant="outline">生成器 {value.generatorVersion || "不可用"}</Badge></div><p className="mt-2 text-xs text-slate-600">来源记录 {value.sourceRecordKey || "不可用"} · 数据版本 {value.dataSource.version}</p><p className="mt-2 text-xs text-slate-500">限制：{value.dataSource.limitations.join("；") || "未声明"}</p><div className="mt-3 flex flex-wrap gap-2">{Object.entries(value.fieldLineage).map(([field,lineage]) => <span key={field} className="rounded-full bg-white px-2 py-1 text-[11px] text-slate-600">{field}: {lineage.provenance}{lineage.method === "unavailable" ? "（不可用）" : ""}</span>)}</div>{isWebSource ? <a className="mt-3 inline-block text-xs font-medium text-indigo-600 hover:underline" href={sourceUri} target="_blank" rel="noreferrer">查看来源说明 ↗</a> : isAuthorizedLocalSource ? <p className="mt-3 text-xs text-slate-600">本地授权来源：{localSourceFiles.join("、") || "未提供文件名"}。原始文件不随系统部署，无法网页预览。</p> : <p className="mt-3 text-xs text-slate-600">内部来源：该来源由系统内部管理，无法通过网页打开。</p>}</section>;
}
