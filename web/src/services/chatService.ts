...
export async function generateRecommendedQuestions(messageId: string) {
  return api.post<never, RecommendedQuestionsPayload>(
    `/conversations/messages/${messageId}/recommended-questions`
  );
}

export async function regenerateTurn(turnId: number) {
  return api.post(`/conversations/turns/${turnId}/regenerate`);
}
