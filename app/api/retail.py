...
def import_retail_data(payload: ImportRequest, db: DbSession, user: CurrentUser) -> ApiResponse:
    ...
    result = _safe(lambda: service.import_baskets(db, int(user.id), Path(payload.source_dir), payload.seed))
    return ApiResponse(data={"importId": result.import_id, "rows": result.rows, "baskets": result.baskets, "products": result.products, "rules": result.rules, "reused": result.reused}, traceId=current_trace_id())


@router.post("/campaigns")
def create_campaign(payload: CampaignRequest, db: DbSession, user: CurrentUser) -> ApiResponse:
    campaign = _safe(lambda: service.create_campaign(db, _owner(db, user), payload.rule_id))
    return ApiResponse(data={"id": campaign.id, "name": campaign.name, "status": campaign.status}, traceId=current_trace_id())


@router.post("/optimization-tasks/{task_id}/transition")
def transition_task(task_id: int, payload: TaskTransitionRequest, db: DbSession, user: CurrentUser) -> ApiResponse:
    task = _safe(lambda: service.transition_task(db, _owner(db, user), task_id, payload.status))
    return ApiResponse(data={"id": task.id, "status": task.status}, traceId=current_trace_id())


@router.get("/reports/weekly")
def weekly_report(db: DbSession, user: CurrentUser) -> ApiResponse:
    content = _safe(lambda: service.report(db, _owner(db, user)))
    return ApiResponse(data={"filename": "instant-retail-weekly-report.md", "content": content}, traceId=current_trace_id())
