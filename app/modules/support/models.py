...
class SupportCase(Base):
    ...
    __table_args__ = (
        UniqueConstraint("owner_id", "case_key", name="uq_support_case_owner_key"),
    )
    ...
    source_data_id: Mapped[int | None] = mapped_column(
        ForeignKey("data_sources.id", name="fk_support_case_data_source"),
        nullable=True,
        index=True,
    )
    ...
    field_lineage_json: Mapped[str] = mapped_column(
        Text, default="{}", server_default=text("'{}'")
    )
    order_id: Mapped[int | None] = mapped_column(
        ForeignKey("orders.id", name="fk_support_case_order"), nullable=True, index=True
    )
...
class SupportMessage(Base):
    ...
    actor_id: Mapped[int | None] = mapped_column(
        ForeignKey("users_v2.id"), nullable=True
    )
...
class KnowledgeRelease(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "owner_id", "version", name="uq_knowledge_release_owner_version"
        ),
    )
...
class KnowledgeReleaseDocument(Base):
    ...
    __table_args__ = (
        UniqueConstraint("release_id", "document_id", name="uq_release_document"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    release_id: Mapped[int] = mapped_column(
        ForeignKey("knowledge_releases.id", ondelete="CASCADE"), index=True
    )
...
class ReplySuggestion(Base):
    ...
    knowledge_release_id: Mapped[int | None] = mapped_column(
...
class ReplyDecision(Base):
    ...
    __table_args__ = (
        UniqueConstraint("suggestion_id", name="uq_reply_decision_suggestion"),
    )
...
class SupportQualityLabel(Base):
    ...
    suggestion_id: Mapped[int | None] = mapped_column(
        ForeignKey("reply_suggestions.id"), nullable=True
    )
...
class KnowledgeGap(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "owner_id", "fingerprint", name="uq_knowledge_gap_owner_fingerprint"
        ),
    )
    ...
    resolving_release_id: Mapped[int | None] = mapped_column(
        ForeignKey("knowledge_releases.id"), nullable=True
    )
...
class SupportReleaseDecision(Base):
    ...
    knowledge_release_id: Mapped[int] = mapped_column(
        ForeignKey("knowledge_releases.id"), index=True
    )
    actor_id: Mapped[int] = mapped_column(ForeignKey("users_v2.id"))
    decision: Mapped[str] = mapped_column(String(20))
...
class SupportEscalation(Base):
    """客服升级：普通客服将复杂/高风险工单升级给主管处理。

    升级生命周期独立于工单生命周期：一个工单可多次升级；
    status 流转 pending → accepted → (returned | transferred | resolved)。
    """
    ...
    raised_by: Mapped[int] = mapped_column(ForeignKey("users_v2.id"))
    ...
    resolution_note: Mapped[str | None] = mapped_column(Text, nullable=True)
    ai_diagnosis_json: Mapped[str] = mapped_column(
